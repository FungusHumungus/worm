...
module.exports = function(query) {
  ...
  function get_by(db, model, options) {
    ...
    return list(db, model, options)
      .then(function(results) {
        ...
        if (results.length > 1) throw new Error('More than one ' + model + ' was returned for ' + where);
        if (results.length === 0) {
          if (options.default)
  ...
  function list(db, model, options) {
    ...
    options = _.defaults(options || {}, {where: '', orderby: '', fetchChildren: true});
    ...
    return query
      ...
      .then(
        function(rows) {
          ...
          var data = options.fetchChildren ? unflatten.unflattenResults (results, schema_) : results;

          results.forEach(function(result) { 
            // Mix in extra properties - methods etc.. of the object
            _.defaults(result, schema_.fields);
